...
summon magma_cube ~ ~ ~ {CustomName:'"Corner"', Tags:["Corner"], Glowing:1b, NoGravity:1b, NoAI:1b, Invulnerable:1b, Size:1b, Silent:1b, AbsorptionAmount:100f}
scoreboard players add @e _age 1
scoreboard players add Global _unique 1
scoreboard players operation @e[type=magma_cube,tag=Corner,scores={_age=1}] _id = Global _unique
...
scoreboard players operation Global mai_scratch1 = @e[type=player,limit=1,scores={mai_scratch0=1..}] pick2
...
execute as @e[type=magma_cube,tag=Corner] at @s run tp ~ ~ ~