...
execute as @e[type=snowball,tag=elevator.engine,tag=elevator.location.maproom] at @s if entity @s[tag=elevator.state.0] run function build:map_specific/elevator/move_up
execute as @e[type=snowball,tag=elevator.engine,tag=elevator.location.maproom] at @s if entity @s[tag=elevator.state.1] run function build:map_specific/elevator/move_down
