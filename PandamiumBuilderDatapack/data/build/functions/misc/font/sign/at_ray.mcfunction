execute store success score <returned> variable if predicate build:in_spawn unless score @s staff_perms matches 2.. run tellraw @s [{"text":"[Sign Font]","color":"dark_red"},{"text":" You cannot edit signs at spawn!","color":"red"}]
...
execute if block ~ ~ ~ #wall_signs run function build:misc/font/sign/get_facing_signs_side
execute if block ~ ~ ~ #build:rotation_signs run function build:misc/font/sign/get_rotation_signs_side

execute if score <returned> variable matches 0 run scoreboard players set <valid_option> variable 0
execute if score <returned> variable matches 0 run function build:misc/font/sign/edit_text
