...
tag @e[limit=1,sort=nearest,distance=..7,type=!player,type=!marker,tag=!Corner,tag=!CloneDisplay,tag=!ClickDetector,tag=!map_specific.fake_block,tag=!custom_block.display,tag=!elevator,predicate=!build:is_riding_an_entity] add selected_entity
