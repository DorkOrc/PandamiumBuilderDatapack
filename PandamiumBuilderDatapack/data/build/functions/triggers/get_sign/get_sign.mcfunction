...
execute if score <can_run> variable matches 1 run data modify storage build:temp pages[0] set from storage build:temp BlockData.front_text.messages[0]
execute if score <can_run> variable matches 1 run data modify storage build:temp pages[1] set from storage build:temp BlockData.front_text.messages[1]
execute if score <can_run> variable matches 1 run data modify storage build:temp pages[2] set from storage build:temp BlockData.front_text.messages[2]
execute if score <can_run> variable matches 1 run data modify storage build:temp pages[3] set from storage build:temp BlockData.front_text.messages[3]
...
execute if score <can_run> variable matches 1 if data storage build:temp {BlockData:{front_text:{has_glowing_text:1b}}} run data modify storage build:temp pages[5] set value 'true'
