...
execute if score @s restart matches -2 run scoreboard players set <restart_countdown> persistent 15
execute if score @s restart matches -2 run function build:misc/restart_countdown

execute if score @s restart matches -3 unless score <restart_countdown> persistent matches 1.. run tellraw @s [{"text":"[Restart]","color":"dark_red"},{"text":" The server is not restarting!","color":"red"}]
execute if score @s restart matches -3 if score <restart_countdown> persistent matches 1.. run tellraw @a [{"text":"[Restart] ","color":"red"},{"selector":"@s"}," cancelled the server restart!"]
execute if score @s restart matches -3 run scoreboard players set <restart_countdown> persistent -1
