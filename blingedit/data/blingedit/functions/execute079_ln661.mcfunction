...
scoreboard players add @e _age 1
scoreboard players add Global _unique 1
execute as @e[type=minecraft:armor_stand,tag=CloneDisplay,scores={_age=1}] run function blingedit:execute079_ln661_branch1
scoreboard players operation @s blingedit.clone_display_source_id = Global _unique
scoreboard players add @e _age 1
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"CloneDisplay\"", Tags:["CloneDisplay"], Glowing:1b, NoGravity:1b, NoAI: 1b, Silent:1b, AbsorptionAmount: 100f, LeftHanded:0b, Invulnerable:1, ShowArms:1b, Pose:{LeftArm:[-90f,0f,0f],RightArm:[-90f,0f,0f]}}
scoreboard players add @e _age 1
scoreboard players add Global _unique 1
execute as @e[type=minecraft:armor_stand,tag=CloneDisplay,scores={_age=1}] run function blingedit:execute079_ln661_branch2
scoreboard players operation @s blingedit.clone_display_dest_id = Global _unique
