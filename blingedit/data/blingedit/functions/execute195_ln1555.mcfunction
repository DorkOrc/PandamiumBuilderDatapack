...
execute as @e[type=minecraft:magma_cube,tag=Corner] if score @s player = @a[scores={mai_scratch0=1..},limit=1] _id run function blingedit:execute190_ln1583
execute as @s run function blingedit:get_minmax
execute as @e[type=minecraft:magma_cube,tag=Corner] if score @s player = @a[scores={mai_scratch0=1..},limit=1] _id run function blingedit:execute194_ln1594