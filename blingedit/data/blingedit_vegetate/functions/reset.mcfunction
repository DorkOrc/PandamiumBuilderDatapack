scoreboard objectives add blingedit.constant dummy
scoreboard players set $5 blingedit.constant 5
scoreboard players set $2 blingedit.constant 2

scoreboard objectives add box_zmax dummy
scoreboard objectives add z dummy
scoreboard objectives add plugin_can_run dummy
scoreboard objectives add x dummy
scoreboard objectives add _age dummy
scoreboard objectives add box_zmin dummy
scoreboard objectives add y dummy
scoreboard objectives add box_ymax dummy
scoreboard objectives add box_xmax dummy
scoreboard objectives add box_ymin dummy
scoreboard objectives add box_xmin dummy
scoreboard objectives add veg dummy
scoreboard objectives add disp_plugins dummy
scoreboard objectives add run_scratch6 dummy
scoreboard objectives add run_scratch5 dummy
scoreboard objectives add run_scratch4 dummy
scoreboard objectives add run_scratch3 dummy
scoreboard objectives add run_scratch2 dummy
scoreboard objectives add run_scratch1 dummy
scoreboard objectives add run_scratch0 dummy
kill @e[type=minecraft:armor_stand,name=RandBasis,scores={RVblingedit_vege=0..}]
scoreboard objectives add RVblingedit_vege dummy
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"RandBasis\"", "Invulnerable":1b, "Invisible":1b, "Marker":1b, "NoGravity":1b}
scoreboard players add @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"RandBasis\"", "Invulnerable":1b, "Invisible":1b, "Marker":1b, "NoGravity":1b}
scoreboard players add @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"RandBasis\"", "Invulnerable":1b, "Invisible":1b, "Marker":1b, "NoGravity":1b}
scoreboard players add @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"RandBasis\"", "Invulnerable":1b, "Invisible":1b, "Marker":1b, "NoGravity":1b}
scoreboard players add @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1
summon minecraft:armor_stand ~ ~ ~ {CustomName:"\"RandBasis\"", "Invulnerable":1b, "Invisible":1b, "Marker":1b, "NoGravity":1b}
scoreboard players add @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1
scoreboard players remove @e[type=minecraft:armor_stand,name=RandBasis] RVblingedit_vege 1