...
# Moving Log
execute if entity @a[x=-18.5,y=-32,z=130,distance=..64,gamemode=!spectator] run function build:map_specific/snapshot_server/moving_log

# Monstrosity Parkour Entrance
particle dust 0 0 1 1 -33 45 -109 1.5 1.5 1.5 1 1

# Nether-Roof Teleporter
execute in build:snapshot/the_nether run particle vibration 0 127 9 200 0.5 60 9.5 0.5 0.5 0.5 1 1
execute in build:snapshot/the_nether positioned 0 68 9 run tp @a[distance=..2] 0 129 9

#schedule function build:map_specific/snapshot_server/main_loop 5t
